...
class Graph
{
  ...
  */
   ...
   * @param {int} weight
   * @return {none}
   * add a new `edge` between two Vertices that are already in the graph
  ...
  /**
   * @param {none}
   * @return {Map} collection of Vertices
   ...
   * return a count of the total number of Vertices in the graph
  ...
  breadthFirst(root)
  {
    ...
    /* Steps:
      ...
      5. add dequeued Vertex to List of `visited Vertices`
      6. and enqueue each of the dequeued Vertex's neighbors
      7. repeat until empty

      Note: disconnected Vertices will never be visited
